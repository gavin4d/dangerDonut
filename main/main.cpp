...
void setup() {
    ...
    if (ADXL.setup(adxl_config)) {
        printf("adxl375 sensor found\n");
    } else {
        printf("error with adxl375\n");
    }
...
//    }
    ...
    adc2_config_channel_atten(ADC2_CHANNEL_1, ADC_ATTEN_DB_6);
...
void loop() {
    ...
    if (myGamepad.isConnected()) {
        ...
        controller_idle = false;//esp_timer_get_time() - myGamepad->getUpdateTime() > CONTROLLER_RESPONSE_TIMEOUT;
        ...
        if (!controller_idle) {
            ...
            float left_power = 0;
            float right_power = 0;
            float y = 0;//-((float)myGamepad->axisY())/512;
            float x = 0;//((float)myGamepad->axisX())/512;
            float r = 0;//((float)myGamepad->axisRX())/512;
            //ESP_LOGI("gamepad", "left stick: x%d, y%d", myGamepad.left_stick_x(), myGamepad.left_stick_y());
            //ESP_LOGI("gamepad", "left trigger: %d", myGamepad.left_trigger());
            ...
            pidOutput = 0;//(float)(myGamepad->throttle() - myGamepad->brake())/1024;
            ...
            if (abs(pidOutput) < SPIN_THRESHOLD) { // run in tank drive if not spinning
                ...
                float theta = atan2f(y,x) + offset[(spinDirection+1)/2]*2*M_PI;
                if (flipped < 0) theta += M_PI; // reverse controls when flipped
            ...
            if (myGamepad.y()) {
                if (!buttons[8]) {
                    ...
                    for (int i = 0; i < NUM_ACCEL_POS; i ++) {
                        ...
                        printf("%s: %f\n", saveLocation, sensor.getAccelPos(i));
                    }
                }
            } else {
                buttons[8] = false;
